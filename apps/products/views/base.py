...
# from apps.products.schema import PRODUCT_MANAGE_METADATA
...
from apps.products.utils.seller import is_product_owner
from apps.products.services import ProductMetaService as meta_services
from apps.products.serializers import (
    ProductMetaDetailSerializer,
    ProductMetaWriteSerializer,
)
from apps.products.tasks import generate_seo_description_for_product
from apps.products.services import ProductMetaService
...
class ProductViewSet(BaseViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            # Cache the full paginated response dictionary using version-based caching
            try:
                ProductListService.set_cached_list(request, response.data)
                logger.info(
                    f"Cached serialized paginated product data: {len(serialized_data)} items"
                )
